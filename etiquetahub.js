...
// Ventas pagadas desde una fecha (para la sección Ventas). Incluye Full, Flex, Colecta y Agencia.
async function sales(conn, fromISO) {
  const out = [];
  const from = fromISO.replace('Z', '-00:00');
  for (let offset = 0; offset < 2000; offset += 50) {
    const r = await api(conn, `/orders/search?seller=${conn.creds.user_id}&order.date_created.from=${encodeURIComponent(from)}&sort=date_desc&limit=50&offset=${offset}`);
    const res = r.results || [];
    for (const o of res) {
      if (!['paid', 'partially_paid', 'partially_refunded'].includes(o.status)) continue;
      out.push({ id: String(o.id), at: o.date_created, amount: Number(o.total_amount || 0), units: (o.order_items || []).reduce((a, i) => a + Number(i.quantity || 1), 0) });
    }
...
// Ventas desde una fecha (sección Ventas). Falabella entrega la hora local de Chile sin zona horaria.
async function sales(conn, fromISO) {
  const after = fromISO.replace(/\.\d{3}Z$/, '+00:00');
  const out = [];
  for (let offset = 0; offset < 3000; offset += 100) {
    const b = await call(conn.creds, 'GetOrders', { CreatedAfter: after, Limit: '100', Offset: String(offset), SortBy: 'created_at', SortDirection: 'DESC' });
    const orders = list(b.Orders, 'Order');
    for (const o of orders) {
      const st = arr(o.Statuses).flatMap(x => (x && typeof x === 'object' ? Object.values(x) : [x])).map(x => String(x).toLowerCase());
      if (st.length && st.every(x => ['canceled', 'cancelled', 'failed', 'returned'].includes(x))) continue;
      const amount = Number(String(o.Price ?? o.GrandTotal ?? 0).replace(/[^0-9.]/g, '')) || 0;
      out.push({ id: String(o.OrderId), localDay: String(o.CreatedAt || '').slice(0, 10), at: o.CreatedAt, amount, units: Number(o.ItemsCount || 1) });
    }
    if (orders.length < 100) break;
  }
  return out;
...
// Ventas desde una fecha (sección Ventas)
async function sales(conn, fromISO) {
  const out = [];
  for (let offset = 0; offset < 3000; offset += 50) {
    const r = await api(conn, `/v3/sub-orders?gteCreatedAt=${encodeURIComponent(fromISO)}&limit=50&offset=${offset}`);
    const subs = flatten(r?.data);
    for (const sub of subs) {
      ...
      const itemsTotal = items.reduce((a, it) => a + Number(it.priceAfterDiscounts ?? it.grossPrice ?? it.price ?? it.basePrice ?? 0) * (it.priceAfterDiscounts != null || it.grossPrice != null ? 1 : Number(it.quantity || 1)), 0);
      const amount = Number(sub.subTotal ?? sub.totalAmount ?? sub.total ?? sub.grossTotal ?? 0) || itemsTotal;
      out.push({ id: String(sub.subOrderNumber), at: sub.originOrderDate || sub.createdAt, amount, units: items.reduce((a, it) => a + Number(it.quantity || 1), 0) || 1 });
    }
    if (subs.length < 50) break;
  }
  return out;
...
// Sección Ventas: guarda lo vendido por día en cada marketplace (hora de Chile)
const cfg = require('./config');
const db = require('./db');
...
async function refresh(force = false) {
  ...
  running = (async () => {
    const fromISO = new Date(Date.now() - 8 * 864e5).toISOString();
    for (const row of db.prepare("SELECT * FROM connections WHERE marketplace IN ('ml','fa','pa')").all()) {
      ...
      if (!c.sales) continue;
      try {
        const list = await c.sales(sync.connObj(row), fromISO);
        const up = db.prepare(`INSERT INTO sales (marketplace, external_id, seller_id, day, amount, units, updated_at) VALUES (?,?,?,?,?,?,datetime('now'))
        ...
        for (const s of list) {
          ...
          if (!day) continue;
          up.run(row.marketplace, s.id, row.seller_id, day, s.amount || 0, s.units || 1);
          seen.add(s.id);
        }
        // ventas que dejaron de estar pagadas (canceladas) en los últimos 7 días se quitan
        const since = localDay(new Date(Date.now() - 7 * 864e5));
        for (const r of db.prepare('SELECT external_id FROM sales WHERE marketplace=? AND seller_id=? AND day>=?').all(row.marketplace, row.seller_id, since)) {
          if (!seen.has(r.external_id)) db.prepare('DELETE FROM sales WHERE marketplace=? AND external_id=?').run(row.marketplace, r.external_id);
        }
      } catch (e) { console.warn('[ventas]', row.marketplace, row.seller_id, e.message); }
    }
    db.prepare('DELETE FROM sales WHERE day < ?').run(localDay(new Date(Date.now() - 40 * 864e5)));
    lastRun = Date.now();
...
function start() {
  if (cfg.demo) {
    ...
    let n = 0;
    for (const s of sellers) for (let i = 0; i < 7; i++) for (const mk of ['ml', 'fa', 'pa']) {
      const k = mk === 'ml' ? 9 : mk === 'fa' ? 4 : 2;
      for (let j = 0; j < Math.floor(Math.random() * k) + 1; j++) ins.run(mk, `demo-${s.id}-${i}-${mk}-${j}-${n++}`, s.id, localDay(new Date(Date.now() - i * 864e5)), 7990 + Math.floor(Math.random() * 25) * 1000, 1 + Math.floor(Math.random() * 2));
    }
    return;
  }
...
module.exports = { refresh, summary, start };
