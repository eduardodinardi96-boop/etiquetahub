...
async function dispatchBy(conn, sh) {
  ...
  if (leadCache.has(sh.id)) return leadCache.get(sh.id);
  let d = null;
  try {
    const lt = await api(conn, `/shipments/${sh.id}/lead_time`);
    d = lt?.estimated_handling_limit?.date || lt?.buffering?.date || null;
  } catch { /* sin dato */ }
  d = d || so.estimated_delivery_time?.date || null;
  leadCache.set(sh.id, d);
  return d;
}
...
async function debugOrder(conn, order) {
  ...
  return { shipment: sh, lead_time: lt };
}
